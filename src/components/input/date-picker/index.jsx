...
const DatePicker = ({
  ...
  if (!control) {
    console.error("DatePicker must be used within a FormHookProvider or FormProvider");
    return null;
  }
    ...
    <Controller
      ...
      render={({ field: { onChange, value, ref, onBlur } }) => (
        <ReactDatePicker
          ref={(elem) => {
            if (elem) {
               // react-datepicker exposes setFocus but we need to pass ref for rhf
               // However, react-datepicker's ref is the component instance, not the input
               // We can ignore ref here or pass it if needed, but RHF usually needs it for focus on error
            }
          }}
          selected={value}
          onChange={onChange}
